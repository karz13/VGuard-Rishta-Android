...
import com.tfl.vguardrishta.domain.GenerateOtpForReVerifyUseCase
 ...
 */
class SendOtpForReVerifyPresenter @Inject constructor(
    private val context: Context,
    private val authenticateUserCaseCase: AuthenticateUserCaseCase,
    private val appVersionUseCase: AppVersionUseCase,
    private val generateOtpForReVerifyUseCase: GenerateOtpForReVerifyUseCase
) : BasePresenter<SendOtpForReVerifyContract.View>(), SendOtpForReVerifyContract.Presenter {
...
//        getView()?.showEnterOtp() // TODO removeTodo
        ...
        disposables?.add(generateOtpForReVerifyUseCase.execute(vguardRishtaUser).applySchedulers()
...
//        user.authType = Constants.LOGIN_TYPE_PASS // TODO removeTodo
//        withDelay(200) {
//            doLoginWitUserNamePassword()  // TODO removeTodo
//        }
        CacheUtils.writeUserCreds(user)

        disposables?.add(generateOtpForReVerifyUseCase.validateLoginOtp(vguardRishtaUser)
            .applySchedulers()
            .doOnSubscribe { getView()?.showProgress() }
            .doFinally { getView()?.stopProgress() }
            .subscribe({
...
//                    getView()?.showToast(it.message)
    ...
    private fun doLoginWitUserNamePassword() {
        disposables?.add(authenticateUserCaseCase.getUser().applySchedulers()
            ...
            .subscribe({
                if (it != null) {
                    if (it?.roleId == CacheUtils.getRishtaUserType()) {
                    //***For AC Engineer
                    if(CacheUtils.getSubUserType()==Constants.AC_SUB_USER_TYPE && it.professionId ==4)
                    {
                        ...
                        if (it.professionId == 0) {
                            getView()?.showMsgDialog(context.getString(R.string.select_usertype_retailer))
                        } else if (it.professionId == 1 || it.professionId == 2 || it.professionId == 3) {
                            getView()?.showMsgDialog(context.getString(R.string.select_usertype_plumber))
                        }
                    }
                    ...
                    if(CacheUtils.getSubUserType()==Constants.EC_SUB_USER_TYPE && (it.professionId ==1 || it.professionId ==2 || it.professionId ==3))
                    {
                        ...
                        if (it.professionId == 0) {
                            ...
                            getView()?.showMsgDialog(context.getString(R.string.select_usertype_ac))
                        }
                    }
                    ...
                    if(CacheUtils.getSubUserType()==Constants.RET_SUB_USER_TYPE && it.professionId ==0)
                    {
                        ...
                        if (it.roleId == Constants.INF_USER_TYPE) {
                            ...
                            //getView()?.showMsgDialog(context.getString(R.string.select_electrician_or_plumber))
                        } else {
                            getView()?.showMsgDialog(context.getString(R.string.select_retailer_login_type))
                        }
                    }
                }
            }, {
    ...
    private fun saveUser(user: VguardRishtaUser?, remember: Boolean) {
        ...
        if (user?.roleId == Constants.INF_USER_TYPE)
        getView()?.navigateToReUpdateKycFragment()
        else
            getView()?.navigateToRetailerReUpdateKycFragment()
    }
}