...
class UpdateKycPresenter @Inject constructor(
    val kycUseCase: KycUpdateUseCase,
    val context: Context,
    val getKycIdTypesUseCase: GetKycIdTypesUseCase

) : BasePresenter<UpdateKycContract.View>(), UpdateKycContract.Presenter {
    ...
    private fun uploadPhotos(kyc: KycDetails) {
    ...
    private fun sendKycDetails(kyc: KycDetails) {
        disposables?.add(kycUseCase.execute(kyc).applySchedulers()
            .doOnSubscribe { getView()?.showProgress() }
            .doFinally { getView()?.stopProgress() }
            .subscribe({
                processResult(it)
